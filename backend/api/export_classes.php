...
// ── API Key Auth ───────────────────────────────────────────────────────────────
// Accept key from Authorization header (preferred) or ?api_key= query param (legacy fallback).
// Header approach prevents the key from appearing in server access logs.
//
//   Preferred:  Authorization: Bearer <key>
//   Fallback:   ?api_key=<key>   (for backward-compat; remove once scheduling system is updated)
...
if (str_starts_with($auth_header, $bearer_prefix)) {
    $header_key = trim(substr($auth_header, strlen($bearer_prefix)));
}

$provided_key = $header_key ?: trim($_GET['api_key'] ?? '');
...
if ($action === 'get_classes') {
    ...
    $res = $conn->query("
        SELECT employee_id AS instructor_id,
    ...
    $res = $conn->query("
        SELECT co.course_id, co.course_code, co.name AS course_name,
               co.credits AS units,
               d.department_id, d.name AS department_name
        FROM   courses co
        LEFT JOIN departments d ON d.department_id = co.department_id
